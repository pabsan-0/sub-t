...
import os
'''
This script will resize all images in a specified directory to a fixed size,
by linearly resizing the height and then cutting the sides to match target width.

Image sizes and paths MUST BE HARDCODED
'''
...
for i, picname in enumerate(os.listdir('./RopePicsRaw/')):
    ...
    if (current_h == target_h) & (current_w == target_w):
        cv2.imwrite(f'./resize_output/squared-{i}.png', pic)

...
DISCONTINUED: unity perception approach is better
'''
...
def overlay_image_alpha(img, img_overlay, pos, alpha_mask):
    ...
    # retrieve number of channels and get sections of alpha masks
    channels = img.shape[2]
    alpha = alpha_mask[y1o:y2o, x1o:x2o]
    alpha_inv = 1.0 - alpha

    # perform weighted sum for every channel based on alphas
...
def get_random_crop(image, crop_height, crop_width):
    ''' Returns a randomly picked square section of a picture. '''
...
if __name__ == '__main__':
    ...
    for j in range(cfg['ouput_num']):
        ...
        for i, item_path in enumerate(fg):
            ...
            w , h  = x2-x1, y2-y1
            imsize = cfg['pic_size']
            cat = choosecat(item_path, cfg['classnames_goofy'])

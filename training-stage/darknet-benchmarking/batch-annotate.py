...
# get text file paths from argv
text_files = argv[1:]


for text_file_path in text_files:

    # define ouput path + init counter 'idx'
    out_dir = text_file_path.split('/')[-1][:-4] + '-infer-results'
    idx = 0

    # Attempt to create the target dir. Hold in a try in case the dir already exists
    try:
        os.mkdir(out_dir)
    except:
        pass
